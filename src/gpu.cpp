...
vector<Vec3> totalVert;
static int tidx;

extern Simulation *glSim;
extern void init_aux_gpu();
extern void save_gpu(const Simulation &sim, int frame);
...
extern void self_mesh(vector<Mesh *> &meshes);
...
extern void delete_constraints(const vector<Constraint*> &cons);
...
void mergeVtxs(const char *ipath, const char *opath)
{
	...
	{//reading pairs
		for (int h = 0; h < glSim->handles.size(); h++) {
			...
			vector<Node *> nodes = ptr->get_nodes();
			if (nodes.size() == 2) {
