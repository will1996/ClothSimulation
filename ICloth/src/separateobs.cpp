...
void find_face_intersection (const Face *face0, const Face *face1) {
    ...
    if (!is_ixn)
        return;
    Vec3 n = -normalize(face0->n/REAL(2.) + SO::nold[get_index(face0,*::obs_meshes)]);
...
bool intersection_midpoint (const Face *face0, const Face *face1,
    ...
    REAL a00 = pos(face0, b00)[axis], a01 = pos(face0, b01)[axis],
           a10 = pos(face1, b10)[axis], a11 = pos(face1, b11)[axis];
    REAL amin = max(min(a00, a01), min(a10, a11)),
...
bool face_plane_intersection (const Face *face, const Face *plane,
    ...
    for (int v = 0; v < 3; v++)
        if (sgn(h[v]) == -sign_sum)
            v0 = v;
    REAL t0 = h[v0]/(h[v0] - h[NEXT(v0)]), t1 = h[v0]/(h[v0] - h[PREV(v0)]);
    b0[v0] = 1 - t0;
    b0[NEXT(v0)] = t0;
    b0[PREV(v0)] = 0;
    b1[v0] = 1 - t1;
...
REAL vf_clear_distance (const Face *face0, const Face *face1, const Vec3 &d,
                          REAL last_dist, Bary &b0, Bary &b1);
REAL ee_clear_distance (const Face *face0, const Face *face1, const Vec3 &d,
                          REAL last_dist, Bary &b0, Bary &b1);
...
REAL vf_clear_distance (const Face *face0, const Face *face1, const Vec3 &d,
    ...
    for (int v = 0; v < 3; v++) {
        ...
        REAL h = dot(xv-x0, n), dh = dot(d, n);
        if (h*dh >= 0)
            continue;
        REAL a0 = stp(x2-x1, xv-x1, d),
...
REAL ee_clear_distance (const Face *face0, const Face *face1, const Vec3 &d,
    ...
    for (int e0 = 0; e0 < 3; e0++) {
        for (int e1 = 0; e1 < 3; e1++) {
            ...
            REAL h = dot(x00-x10, n), dh = dot(d, n);
            if (h*dh >= 0)
                continue;
            REAL a00 = stp(x01-x10, x11-x10, d),
...
REAL SeparationOpt::objective (const double *x) const {
    REAL f = 0;
    for (int n = 0; n < nodes.size(); n++) {
...
REAL SeparationOpt::constraint (const double *x, int j, int &sign) const {
...
void SeparationOpt::con_grad (const double *x, int j, double factor,
    ...
    for (int v = 0; v < 3; v++) {
        int i0 = find(ixn.f0->v[v]->node, nodes),
            i1 = find(ixn.f1->v[v]->node, nodes);
        if (i0 != -1)
            add_subvec(grad, i0, REAL(factor)*ixn.b0[v]*ixn.n);
        if (i1 != -1)
            add_subvec(grad, i1, -REAL(factor)*ixn.b1[v]*ixn.n);
    }
}
