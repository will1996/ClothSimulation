...
REAL get_mass(const Node *node) { return is_free(node) ? node->m : obs_mass; }
...
struct gNormalOpt : public NLConOpt {
	...
	REAL constraint(const double *x, int i, int &sign) const;
...
REAL gNormalOpt::objective(const double *x) const {
	REAL e = 0;
	for (int n = 0; n < _zone->_nodes.size(); n++) {
...
void gNormalOpt::obj_grad(const double *x, double *grad) const {
	for (int n = 0; n < _zone->_nodes.size(); n++) {
		...
		bool f = _zone->_frees[n];
		Vec3 dx = get_x(nd, f) - get_xold(nd, f);
		set_subvec(grad, n, _inv_m*get_mass(nd, f)*dx);
	}
}

REAL
gNormalOpt::constraint(const double *x, int j, int &sign) const
{
	sign = 1;
	REAL c = -::thickness;
...
gNormalOpt::con_grad(const double *x, int j, double factor, double *grad) const
{
	...
	for (int n = 0; n < 4; n++) {
		...
		if (i != -1)
			add_subvec(grad, i, REAL(factor*impact._w[n]) * impact._n);
	}
}
