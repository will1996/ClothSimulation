...
Vec4 evaluate_stretching_sample (const Mat2x2 &_G, const StretchingData &data) {
    ...
    Vec2 V = eig.Q.col(0);
    REAL angle_weight=fabsf(atan2f(V[1], V[0])/M_PI)*8;
    ...
    strain_id=0;
    REAL strain_weight=strain_value-strain_id;
    Vec4 real_elastic;
    real_elastic=
...
Vec4 stretching_stiffness (const Mat2x2 &G, const StretchingSamples &samples) {
    REAL a=(G(0,0)+0.25)*nsamples;
    REAL b=(G(1,1)+0.25)*nsamples;
    REAL c=fabsf(G(0,1))*nsamples;
    a=clamp(a, REAL(0.), REAL(nsamples-1-1e-5));
	b = clamp(b, REAL(0.), REAL(nsamples - 1 - 1e-5));
	c = clamp(c, REAL(0.), REAL(nsamples - 1 - 1e-5));
    int ai=(int)floor(a);
    int bi=(int)floor(b);
    int ci=(int)floor(c);
    if(ai<0)        ai=0;
...
REAL bending_stiffness (const Edge *edge, int side,
                          const BendingData &data, REAL initial_angle) {
    REAL curv = edge->theta*edge->l/(edge->adjf[0]->a + edge->adjf[1]->a);
    REAL alpha = curv/2;
    REAL value = alpha*0.2; // because samples are per 0.05 cm^-1 = 5 m^-1
    if(value>4) value=4;
    ...
    REAL    bias_angle=(atan2f(du[1], du[0])+initial_angle)*4/M_PI;
